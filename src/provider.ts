...
import { EthError, convertSafeTxToEthersTx, getLowerCase, poll } from "./utils";
import SafeAppsSDK, { SafeInfo } from "@gnosis.pm/safe-apps-sdk";
import { hexValue, hexlify, isHexString } from "@ethersproject/bytes";
...
export class SafeAppsSdkSigner extends Signer {
  ...
  async signMessage(): Promise<string> {
    return logger.throwError(
      "signing messages is not supported",
      Logger.errors.UNSUPPORTED_OPERATION,
      {
        operation: "connect",
      }
  ...
  signTransaction(): Promise<string> {
    return logger.throwError(
      "signing transactions is not supported",
      Logger.errors.UNSUPPORTED_OPERATION,
      {
...
export class SafeAppsSdkProvider extends BaseProvider {
  ...
  async send(method: string, params: any): Promise<any> {
    switch (method) {
      ...
      case "getBalance":
        return this._sdk.eth.getBalance([
          getLowerCase(params.address),
          params.blockTag,
        ]);

      case "getCode":
        return this._sdk.eth.getCode([
          getLowerCase(params.address),
          params.blockTag,
        ]);

      case "getStorageAt":
        return this._sdk.eth.getStorageAt([
          getLowerCase(params.address),
          params.position,
          params.blockTag,
        ]);
      ...
      case "getBlock":
        if (params.blockTag) {
          return this._sdk.eth.getBlockByNumber([
            params.blockTag,
            !!params.includeTransactions,
          ]);
        } else if (params.blockHash) {
          return this._sdk.eth.getBlockByHash([
            params.blockHash,
            !!params.includeTransactions,
          ]);
        }
        return logger.throwError(
          "Invalid getBlock arguments",
  ...
  static hexlifyTransaction(
    ...
    (["gasLimit", "gasPrice", "nonce", "value"] as const).forEach(function (
      key
    ) {
